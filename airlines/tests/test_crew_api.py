...
from airlines.serializers import FlightListSerializer, AirportDetailSerializer

AIRPORT_URL = reverse("airlines:airport-list")


def detail_url(airport_id: int):
    return reverse("airlines:airport-detail", args=[airport_id])
...
class AirportApiTest(TestCase):
    ...
    def test_create_forbidden_for_not_admin(self):
        payload = {
            "name": "Test",
            "close_big_city": "Test city",
            }
...
class AdminAirportTest(TestCase):
    def setUp(self) -> None:
        ...
        self.user = get_user_model().objects.create_user(
            "test1@test.com", "test1234", is_staff=True
        )
        self.client.force_authenticate(self.user)
    ...
    def test_create_airport(self):
        ...
        self.assertEqual(payload["close_big_city"], getattr(airport, "close_big_city"))

    def test_destroy_airport_not_allowed(self):
        airport = sample_airport()
        url = detail_url(airport.id)
